...
export default function Price() {
  ...
  return (
    ...
    <button className="priceadd-button" onClick={addGalleryItem}>Add Photo</button>
    <div className="priceimage-grid">
        {galleryItems.map(item => (
            <div key={item.id} className="pricecard">
                <img src={item.imageUrl} alt={item.title} />
                <div className="pricecard-details">
                    <h3>{item.title}</h3>
                    <p>{item.description}</p>
                    <div className="pricecard-buttons">
